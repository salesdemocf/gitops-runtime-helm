...
Usage:
{{ include "cf-common-0.19.2.container" (dict "Values" .Values.container "context" $) }}
*/}}
{{-  define "cf-common-0.19.2.container" -}}
...
{{- $containerName = include "cf-common-0.19.2.tplrender" (dict "Values" .Values.nameOverride "context" $) -}}
{{- end }}

- name: {{ $containerName }}
  image: {{ (include "cf-common-0.19.2.tplrender" (dict "Values" (include "cf-common-0.19.2.image.name" (dict "image" .Values.image "context" $)) "context" $)) }}
  ...
  {{- with .Values.command }}
    ...
    {{- end }}
  command: {{- include "cf-common-0.19.2.tplrender" (dict "Values" . "context" $) | nindent 2 }}
  {{- end }}
  ...
  envFrom:
    {{- with .Values.envFrom }}
      ...
      {{- end }}
      {{- include "cf-common-0.19.2.tplrender" (dict "Values" . "context" $) | trim | nindent 4 }}
    {{- end }}
    {{- range $secretName, $secretItem := $.Values.secrets }}
      {{- if $secretItem.enabled }}
    - secretRef:
        name: {{ printf "%s-%s" (include "cf-common-0.19.2.names.fullname" $) $secretName }}
      {{- end }}
    {{- end }}
  {{- end }}
...
*/}}
  ...
  {{- $mergedEnv = merge $mergedEnv $.Values.global.env }}
    {{- end }}
  env:
  {{- include "cf-common-0.19.2.env-vars" (dict "Values" $mergedEnv "context" $) | trim | nindent 2 }}
  {{- end }}

  {{- include "cf-common-0.19.2.ports" $ | trim | nindent 2 }}
...
*/}}
  ...
  {{- $mergedVolumeMounts = mergeOverwrite $mergedVolumeMounts $.Values.volumeMounts }}
    {{- end }}
  volumeMounts: {{ include "cf-common-0.19.2.volumeMounts" (dict "Values" $mergedVolumeMounts "context" $) | trim | nindent 2 }}

  {{- with .Values.probes }}
  {{- include "cf-common-0.19.2.probes" . | trim | nindent 2 }}
  {{- end }}
